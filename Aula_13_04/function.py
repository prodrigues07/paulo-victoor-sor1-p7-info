...
def showProducts():
    ...
    def table(product, entry, left, stock, price, total):
        res=("|{:<19s}|{:>13d}|{:>13d}|{:>13d}|{:>13.2f}|{:>13.2f}|".format(product, entry, left, stock, price, total))
        res1=("+-------------------+-------------+-------------+-------------+-------------+-------------+")
    ...
    res2=("+-------------------+-------------+-------------+-------------+-------------+-------------+")
    res3=("|{:<19s}|{:>13s}|{:>13s}|{:>13s}|{:>13s}|{:>13s}|".format('Lista de Produtos', 'QTD Entradas', 'QTD Saídas',
    ...
    res5=("|{:<19s}|{:>13s}|{:>13s}|{:>13s}|{:>13s}|{:>13.2f}|".format('', '', '', '', 'Total:', 1750))
    res6=("+-------------------+-------------+-------------+-------------+-------------+-------------+")

    lbl1 = Label(window, text="Hello")
    lbl1.grid(column=0, row=0)

    txt1 = Entry(window, width=0)
    txt1.grid(column=0, row=0)

    lbl1.configure(text1=res2)
    lbl1.configure(text1=res3)
    lbl1.configure(text1=res4)
    lbl1.configure(text1=res5)
    lbl1.configure(text1=res6)
